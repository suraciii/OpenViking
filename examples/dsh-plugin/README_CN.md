...
- **提交触发记忆提取(commit)** — 每个 `turn/end` 冲刷捕获消息,每 `commitTurnThreshold` 个回合、服务器 pending token 超过 `commitTokenThreshold` 时、以及 agent 销毁时(限时 3 秒)提交(触发服务端记忆提取);常规提交尊重 `commitKeepRecentCount`(保留最新原始消息,与其它 harness 插件一致),销毁提交强制 `0`,短会话在会话边界立即全量提取。
